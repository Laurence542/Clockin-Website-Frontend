...
const TimeOffRequestsList = () => {
  const { userId } = useParams(); // target user's id (from URL)
  ...
  useEffect(() => {
    const fetchUserDetail = async () => {
      try {
        // Using the existing route to fetch the target user's details including timeOffRequests
        const response = await axios.get(
          `${process.env.REACT_APP_API_URL}/api/users/${userId}`,
          { withCredentials: true }
        );
        const userDetail = response.data;
        const sortedRequests = (userDetail.timeOffRequests || []).sort(
          (a, b) =>
            new Date(b.submittedAt || b.startDate) -
            new Date(a.submittedAt || a.startDate)
        );
